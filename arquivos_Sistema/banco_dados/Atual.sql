...
--
-- Host: 127.0.0.1
-- Tempo de geração: 06/08/2025 às 15:26
...
--
-- Estrutura para tabela `estoque_esgotado`
--
CREATE TABLE `estoque_esgotado` (
...
CREATE TABLE `pedidos` (
  `idpedidos` varchar(150) PRIMARY KEY NOT NULL,
  ...
  `gravacaoInternaM` varchar(120) DEFAULT NULL,
  `gravacaoInternaF` varchar(120) DEFAULT NULL,
...
--
-- Estrutura para Usuarios `usuarios`
...
--
-- Estrutura para tabela `reabastecer_estoque_polimento`
--

CREATE TABLE `reabastecer_estoque_polimento` (
  `nome` varchar(150) NOT NULL,
  `descricaoEstoque` text NOT NULL,
  `imagem` varchar(150) NOT NULL,
  `peso` decimal(4,2) NOT NULL,
...
--
ALTER TABLE `pedidos`
  ADD KEY `fk_pedidos_estoque` (`estoque`);
...
--
ALTER TABLE `pedidos`
  ADD CONSTRAINT `fk_pedidos_estoque` FOREIGN KEY (`estoque`) REFERENCES `estoque` (`nome`);
...
CREATE DEFINER=`rainhaBanco`@`localhost` EVENT `Excluir_PE` ON SCHEDULE EVERY 1 DAY STARTS '2025-05-04 14:56:07' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM pedidospe
WHERE STR_TO_DATE(data_digitada, '%Y-%m-%d') < CURDATE() - INTERVAL 365 DAY;

CREATE DEFINER=`rainhaBanco`@`localhost` EVENT `Excluir_PF` ON SCHEDULE EVERY 1 DAY STARTS '2025-05-04 14:56:07' ON COMPLETION NOT PRESERVE ENABLE COMMENT 'EVENTO PARA APAGAR DADOS ANTIGOS DO BANCO' DO DELETE FROM pedidosp 
WHERE STR_TO_DATE(data_digitada, '%Y-%m-%d') < CURDATE() - INTERVAL 365 DAY;

CREATE DEFINER=`rainhaBanco`@`localhost` EVENT `Excluir_PG` ON SCHEDULE EVERY 1 DAY STARTS '2025-05-04 14:56:07' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM pedidospg
WHERE STR_TO_DATE(data_digitada, '%Y-%m-%d') < CURDATE() - INTERVAL 365 DAY;
