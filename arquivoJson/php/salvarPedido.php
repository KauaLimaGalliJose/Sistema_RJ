...
if (!isset($pedidos[$data['nome']])) {
    // Se ainda não existe, cria novo
    $pedidos[$data['nome']] = [];
}

if ($data['valor'] !== 'Indefinido') {
    // Se o valor não for "Indefinido", atualiza o estado
    $pedidos[$data['nome']]['estado'] = $data['valor'];
}

// Atualiza os outros campos sempre
$pedidos[$data['nome']]['conta'] = $data['conta'] ?? "";
$pedidos[$data['nome']]['gravacao'] = $data['gravacao'] ?? "";
$pedidos[$data['nome']]['data'] = $data['dataAtual'] ?? "";
$pedidos[$data['nome']]['horaEntrega_Escritorio'] = $data['horaEntrega'] ?? "";
